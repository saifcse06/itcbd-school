        ...
        if (($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if ($_POST) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    // newly added by shakhawat

                    $array['bangla_name']        = $this->input->post('bangla_name');
                    $array['birth_certificate']  = $this->input->post('birth_certificate');
                    $array['fathers_name']       = $this->input->post('fathers_name');
                    $array['fathers_profession'] = $this->input->post('fathers_profession');
                    $array['fathers_nid']        = $this->input->post('fathers_nid');
                    $array['mothers_name']       = $this->input->post('mothers_name');
                    $array['mothers_profession'] = $this->input->post('mothers_profession');
                    $array['mothers_nid']        = $this->input->post('mothers_nid');

                    // fathers mothers file

                    // address related field
                    $array['present_village']    = $this->input->post('present_village');
                    $array['present_thana']      = $this->input->post('present_thana');
                    $array['present_district']   = $this->input->post('present_district');
                    $array['permanent_village']  = $this->input->post('permanent_village');
                    $array['permanent_thana']    = $this->input->post('permanent_thana');
                    $array['permanent_district'] = $this->input->post('permanent_district');

                    // stipend related field
                    $array['stipend']              = $this->input->post('stipend');
                    $array['merit_stipend']        = $this->input->post('merit_stipend');
                    $array['others_stipend']       = $this->input->post('others_stipend');
                    $array['orphan']               = $this->input->post('orphan');
                    $array['physically_chalanged'] = $this->input->post('physically_chalanged');
                    $array['freedom_fighter']      = $this->input->post('freedom_fighter');
                    $array['poor_fund']            = $this->input->post('poor_fund');

                    // gurdian
                    $array['gurdian_profession']              = $this->input->post('gurdian_profession');
                    $array['gurdian_phone_no']                = $this->input->post('gurdian_phone_no');
                    $array['gurdian_without_father']          = $this->input->post('gurdian_without_father');
                    $array['relation_with_student']           = $this->input->post('relation_with_student');
                    $array['gurdian_without_father_phone_no'] = $this->input->post('gurdian_without_father_phone');
                    // previous school
                    $array['previous_school'] = $this->input->post('previous_school');
                    $array['previous_class']  = $this->input->post('previous_class');
                    $array['previous_grade']  = $this->input->post('previous_grade');
                    ...
                    if (isset($_FILES['fathers_file']) && is_uploaded_file($_FILES['fathers_file']['tmp_name'])) {
                        $father_file_config['upload_path']   = './uploads/images';
                        $father_file_config['allowed_types'] = 'gif|jpg|png|jpeg|webp';
                        $father_file_config['max_size']      = 1000;
                        $father_file_config['max_width']     = 3000;
                        $father_file_config['encrypt_name']  = true;
                        $father_file_config['max_height']    = 3000;
                        $this->load->library('upload', $father_file_config);

                        if (!$this->upload->do_upload('fathers_file')) {
                            $error = array('error' => $this->upload->display_errors());
                        } else {
                            $data                  = $this->upload->data();
                            $array['fathers_file'] = $data['file_name'];
                        }

                    }

                    // mothers file
                    if (isset($_FILES['mothers_file']) && is_uploaded_file($_FILES['mothers_file']['tmp_name'])) {
                        $father_file_config['upload_path']   = './uploads/images';
                        $father_file_config['allowed_types'] = 'gif|jpg|png|jpeg|webp';
                        $father_file_config['max_size']      = 1000;
                        $father_file_config['max_width']     = 3000;
                        ...
                        if (!$this->upload->do_upload('mothers_file')) {
                            $error = array('error' => $this->upload->display_errors());
                        } else {
                            $data                  = $this->upload->data();
                            $array['mothers_file'] = $data['file_name'];
                        }
                        // file
                    }
            ...
            if ((int) $studentID && (int) $url) {
                ...
                if (customCompute($this->data['student'])) {
                    if ($_POST) {
                        ...
                        $rules = $this->rules();
                        // unset password rules during update
                        unset($rules[20]);
                        $this->form_validation->set_rules($rules);
                        if ($this->form_validation->run() == false) {
                            ...
                            if (isset($_FILES['fathers_file']) && is_uploaded_file($_FILES['fathers_file']['tmp_name'])) {
                                ...
                                if (file_exists(FCPATH . 'uploads/images/' . $this->data['student']->fathers_file)) {
                                    unlink(FCPATH . 'uploads/images/' . $this->data['student']->fathers_file);
                                }
                                $father_file_config['upload_path']   = './uploads/images';
                                $father_file_config['allowed_types'] = 'gif|jpg|png|jpeg|webp';
                                $father_file_config['max_size']      = 1000;
                            ...
                            if (isset($_FILES['mothers_file']) && is_uploaded_file($_FILES['mothers_file']['tmp_name'])) {
                                ...
                                if (file_exists(FCPATH . 'uploads/images/' . $this->data['student']->mothers_file)) {
                                    unlink(FCPATH . 'uploads/images/' . $this->data['student']->mothers_file);
                                }
